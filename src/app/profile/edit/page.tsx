...
export default function EditProfilePage() {
  ...
  const handleAddHobby = () => {
    ...
    setFormData({
      ...formData,
      hobbies: [...formData.hobbies, { name: val, emoji: '' }]
    });
    setNewHobbyInput('');
  };
      ...
      <div className="max-w-2xl mx-auto px-4 pb-20">
        ...
        <form onSubmit={handleSave} className="space-y-6">
          ...
          <div className="card-minimal">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-2">สีธีมประจำตัว</label>
              ...
              <div><label className="block text-sm font-medium mb-2 flex items-center gap-2"><Music size={16} /> ลิงก์เพลง (YouTube)</label><input type="url" value={formData.music_url} onChange={(e) => setFormData({ ...formData, music_url: e.target.value })} className="input-minimal" placeholder="https://youtube.com/..." /></div>
            </div>
          </div>
              ...
              <div className="flex flex-wrap gap-2">
                {formData.hobbies.map((h, i) => (
                  <div key={i} className="flex items-center gap-2 px-3 py-2 bg-gray-100 rounded-xl font-bold text-xs">
                    <span>{h.name}</span>
                    <button type="button" onClick={() => setFormData({...formData, hobbies: formData.hobbies.filter((_, idx) => idx !== i)})} className="text-red-500 font-black">×</button>
                  </div>
                ))}
              </div>
            </div>
