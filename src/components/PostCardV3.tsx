...
export default function PostCardV3({ post: initialPost, currentUserId, onDelete, profileOwnerId }: PostCardProps) {
  ...
  const handleViewLikers = async () => {
    ...
    try {
      ...
      if (likesData && likesData.length > 0) {
        const { data: usersData } = await supabase.from('users').select('id, username, display_name, profile_img_url').in('id', likesData.map((d: any) => d.user_id));
        setLikers(usersData || []);
      }
  ...
  const handleDeleteComment = async (c: Comment) => {
    ...
    try {
      ...
      await supabase.from('notifications').delete().eq('post_id', post.id).eq('sender_id', c.author_id).in('type', ['comment', 'reply', 'tag_comment']);
  ...
  const MentionMenu = () => {
    ...
    return (
      <div className="absolute bottom-full left-0 mb-2 w-64 bg-white border border-gray-100 rounded-2xl shadow-xl z-[100] overflow-hidden animate-in slide-in-from-bottom-2">
        <div className="p-2 border-b bg-gray-50 flex items-center gap-2">
          <AtSign size={12} className="text-frog-500" />
          <span className="text-[10px] font-black uppercase text-gray-500 tracking-widest">แท็กเพื่อน</span>
        </div>
        {filteredFriends.map(f => (
          <button key={f.id} onClick={() => insertMention(f)} className="w-full flex items-center gap-3 p-3 hover:bg-frog-50 transition-colors text-left border-b border-gray-50 last:border-0">
            ...
            <div className="min-w-0">
              <p className="text-sm font-bold text-gray-900 truncate">{f.display_name}</p>
            ...
            {isEditing ? (
              <div className="space-y-2 relative">
                <textarea 
                  ...
                  onChange={(e) => handleInputChange(e.target.value, 'edit_comment')}
            ...
            {replyTo === c.id && (
              <div className="mt-3 space-y-2 animate-in slide-in-from-left-2 relative">
                <div className="space-y-2">
                  ...
                  {showReplyImageInput && (
                    <input type="url" value={replyImageUrl} onChange={(e) => setReplyImageUrl(e.target.value)} placeholder="ใส่ลิงก์รูปภาพที่นี่ (https://...)" className="w-full text-[10px] py-1.5 px-3 bg-gray-50 border border-dashed border-gray-200 rounded-lg outline-none" />
                  )}
                </div>
              </div>
            )}
    ...
    <div className="card-minimal border border-gray-100 shadow-sm relative">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap text-sm">
            ...
            {post.target && post.target.id !== post.author?.id && (
              <><ChevronRight size={14} className="text-gray-400" /><Link href={`/profile/${post.target.username}`} className="font-black text-gray-900 hover:text-frog-600">{post.target.display_name}</Link></>
            )}
          </div>
      ...
      {isEditingPost ? (
        <div className="mb-4 space-y-3 animate-in fade-in">
          <textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} className="w-full text-sm p-4 bg-gray-50 border border-gray-200 rounded-2xl outline-none focus:ring-2 focus:ring-frog-500 focus:bg-white min-h-[120px] transition-all" autoFocus placeholder="คุณกำลังคิดอะไรอยู่..." />
      ...
      <div className="flex items-center gap-6 pt-3 border-t border-gray-50">
        <div className="flex items-center gap-1.5">
          <button onClick={handleLike} className={`transition-all active:scale-75 p-1 -ml-1 rounded-full ${isLiked ? 'text-red-500' : 'text-gray-400 hover:bg-red-50 hover:text-red-400'}`}><Heart className={`w-5 h-5 ${isLiked ? 'fill-current' : ''}`} /></button>
          <button onClick={handleViewLikers} className={`text-xs font-black transition-colors py-1 pr-2 ${likeCount > 0 ? 'text-gray-500 hover:text-gray-900 hover:underline cursor-pointer' : 'text-gray-400 cursor-default'}`}>{likeCount}</button>
        </div>
        <button onClick={() => setShowComments(!showComments)} className={`flex items-center gap-2 text-xs font-black transition-colors p-1 -ml-1 rounded-lg ${showComments ? 'text-frog-600 bg-frog-50' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-600'}`}><MessageCircle className="w-5 h-5" /> {commentCount}</button>
      </div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-50 space-y-4 animate-in fade-in duration-300">
          {isCommentsLoading ? (
            ...
            <>
              <form onSubmit={handleComment} className="space-y-2 relative">
                <div className="flex gap-2 relative">
                  <div className="relative flex-1">
                    <input type="text" value={newComment} onChange={(e) => handleInputChange(e.target.value, 'comment')} placeholder="เขียนความคิดเห็น..." className="input-minimal w-full text-sm py-2 px-4 bg-gray-50 border border-gray-100 rounded-xl outline-none focus:ring-2 focus:ring-frog-200 transition-all" disabled={isSubmitting} />
              ...
              <div className="space-y-3">{comments.length === 0 ? <p className="text-center text-gray-300 text-[10px] font-black uppercase py-4 tracking-widest">ยังไม่มีความคิดเห็น</p> : comments.map(c => renderCommentItem(c))}</div>
            </>
          )}
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in" onClick={() => setShowLikersModal(false)}>
          <div className="bg-white rounded-[2rem] w-full max-w-sm overflow-hidden shadow-2xl p-6 animate-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
            ...
            <div className="max-h-[50vh] overflow-y-auto pr-2 space-y-2 no-scrollbar">
              {isLoadingLikers ? <div className="flex justify-center py-8"><Loader2 className="w-6 h-6 animate-spin text-frog-500" /></div> : likers.length === 0 ? <p className="text-center text-xs font-bold text-gray-400 py-6 bg-gray-50 rounded-2xl border border-dashed border-gray-200">ยังไม่มีผู้กดถูกใจ</p> : likers.map((user, idx) => (
                  <Link key={idx} href={`/profile/${user.username}`} className="flex items-center gap-3 p-3 hover:bg-gray-50 rounded-2xl transition-colors group border border-transparent hover:border-gray-100">
                    ...
                    <div className="flex-1 min-w-0"><p className="font-bold text-sm text-gray-900 truncate group-hover:text-frog-600 transition-colors">{user.display_name}</p><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest truncate">@{user.username}</p></div>
                  </Link>
                ))
              }
            </div>
