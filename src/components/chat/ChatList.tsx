  ...
  const handleCreateChat = async (friendId?: string) => {
    ...
    try {
      if (mode === 'single' && friendId) {
        // ✅ ใช้ RPC ที่สร้างไว้ใน Database เพื่อเช็กห้องที่มีอยู่แล้ว (กันการสร้างห้องขยะ)
    ...
    <div className="h-full flex flex-col bg-white relative border-r">
      <div className="p-4 border-b space-y-4">
        ...
        <div className="px-1 relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="ค้นหาการสนทนา..." className="w-full pl-10 pr-4 py-2.5 bg-gray-100 rounded-2xl text-sm outline-none focus:ring-2 focus:ring-frog-200 transition-all text-gray-900" />
        </div>
      </div>
          ...
          filtered.map(c => {
            ...
            return (
              <button 
                ...
                className={`w-full p-4 flex gap-3 hover:bg-gray-50 transition-all border-b border-gray-50 ${selectedChatId === c.id ? 'bg-frog-50/50' : ''}`}
              >
                ...
                {c.unread_count > 0 && <div className="bg-frog-500 text-white text-[10px] rounded-full px-1.5 h-4 flex items-center justify-center font-black min-w-[16px]">{c.unread_count}</div>}
              </button>
            );
          })
        )}
      ...
      {showCreateModal && (
        <div className="absolute inset-0 bg-white z-[60] flex flex-col animate-in fade-in slide-in-from-bottom-2">
          <div className="p-4 border-b flex justify-between items-center bg-gray-50">
            <span className="font-black text-[10px] uppercase text-frog-600 tracking-widest">{mode === 'single' ? 'New Message' : 'Create Group'}</span>
          ...
          {mode === 'group' && (
            <div className="p-4 bg-white border-b space-y-3">
               <div className="flex items-center gap-4">
                  <div className="w-16 h-16 bg-gray-100 rounded-2xl flex items-center justify-center text-gray-400 border-2 border-dashed border-gray-200 overflow-hidden">
                    {groupImg ? <img src={groupImg} className="w-full h-full object-cover" alt="" /> : <Camera size={24}/>}
                  </div>
                  <div className="flex-1 space-y-2">
                    <input value={groupName} onChange={e => setGroupName(e.target.value)} placeholder="ชื่อกลุ่ม..." className="w-full p-2 bg-gray-50 rounded-xl text-sm outline-none border border-transparent focus:border-frog-200 transition-all text-gray-900" />
                    <input value={groupImg} onChange={e => setGroupImg(e.target.value)} placeholder="URL รูปภาพกลุ่ม..." className="w-full p-2 bg-gray-50 rounded-xl text-[11px] outline-none border border-transparent focus:border-frog-200 transition-all text-gray-900" />
                  </div>
               </div>
            </div>
          )}
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            {loadingFriends ? (
              <div className="py-20 flex justify-center"><Loader2 className="animate-spin text-frog-500" /></div>
            ) : friends.length === 0 ? (
              ...
              friends.map(f => (
                <button 
                  ...
                  className={`w-full p-3 flex items-center gap-3 border-b border-gray-50 transition-colors ${selectedFriendIds.includes(f.id) ? 'bg-frog-50' : 'hover:bg-gray-50'}`}
                >
                  <div className="relative">
                    ...
                    {selectedFriendIds.includes(f.id) && <div className="absolute -top-1 -right-1 bg-frog-500 text-white rounded-full p-0.5 border-2 border-white shadow-sm"><Check size={10} strokeWidth={4}/></div>}
                  </div>
          ...
          {mode === 'group' && (
            <div className="p-4 bg-gray-50 border-t">
              <button 
                ...
                className="w-full py-4 bg-frog-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-lg shadow-frog-100 disabled:opacity-50 active:scale-95 transition-all"
              >
