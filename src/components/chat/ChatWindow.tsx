...
export default function ChatWindow({ chatId, currentUser, onBack, onRefreshChats }: ChatWindowProps) {
  ...
  // ✅ ใช้ Refs เพื่อให้ Real-time เข้าถึงข้อมูลล่าสุดได้โดยไม่ต้องยิง DB ซ้ำ
  ...
  const loadChatData = async () => {
    try {
      // ✅ 1. มัดรวมการดึงข้อมูลพื้นฐานแชท (Parallel) เพื่อลด Waterfall
      ...
      // ✅ 3. ดึงข้อความ (ไม่ต้อง Join table users ทุกบรรทัดเพื่อความไว)
      ...
      if (messagesData) {
        ...
        // Map ข้อมูลผู้ส่งจากตัวแปรที่มีอยู่แล้ว (ไม่ต้องยิง DB ซ้ำ 30 รอบ)
  ...
  const setupRealtimeSubscription = () => {
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        // ✅ Optimization: ไม่ต้องวิ่งไปดึงข้อมูล User จากตารางใหม่
        // เพราะในแชท 1-1 คนส่งมีแค่เรากับเขา ซึ่งเรามีข้อมูลอยู่แล้ว
      ...
      .on('postgres_changes', {
        ...
        const sender = updatedMessage.sender_id === currentUserRef.current.id 
          ? currentUserRef.current 
          : otherUserRef.current;
  ...
  const saveNicknames = async () => {
    ...
    try {
      ...
      const promises = [];
      if (myNewNick) {
        promises.push(supabase.from('chat_nicknames').upsert({
          chat_id: chatId, target_user_id: currentUser.id, nickname: myNewNick,
        }, { onConflict: 'chat_id,target_user_id' }));
      } else {
        promises.push(supabase.from('chat_nicknames').delete().eq('chat_id', chatId).eq('target_user_id', currentUser.id));
      }
      ...
      if (otherNewNick) {
        ...
        promises.push(supabase.from('chat_nicknames').delete().eq('chat_id', chatId).eq('target_user_id', otherUser.id));
      }
  ...
  return (
    <div className="flex flex-col flex-1 bg-white md:rounded-2xl md:shadow-sm md:border md:border-gray-200 overflow-hidden h-full min-h-0 w-full">
      ...
      {/* Header */}
      <div className="flex-shrink-0 p-4 border-b flex items-center gap-3 bg-white z-10"
                ...
                <div className="flex items-center gap-3 pt-3 border-t border-gray-100">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => { saveThemeColor(themeColor); setShowColorPicker(false); }}
                        className="h-8 px-3 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-xs font-bold transition flex-shrink-0">
                        ยืนยัน
                      </button>
                    </div>
                  </div>
            ...
            <>
              ...
              <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-xl shadow-xl border border-gray-200 z-20 overflow-hidden animate-in zoom-in-95 duration-200">
                ...
                <button onClick={handleDeleteHistory}
                  className="w-full px-4 py-3 text-left text-sm hover:bg-gray-50 flex items-center gap-2 text-red-600 border-t border-gray-50">
      ...
      <div 
        ...
        className="flex-1 overflow-y-auto p-4 space-y-4 relative bg-slate-50/30"
      >
        {isLoadingMore && (
        ...
        {messages.length === 0 && !isLoadingMore ? (
          <div className="flex flex-col items-center justify-center h-full text-gray-400">
            ...
            <p className="font-bold text-gray-400">ยังไม่มีข้อความ</p>
            <p className="text-xs mt-1">เริ่มสนทนากันเลย!</p>
          </div>
        ) : (
          <>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-[2rem] w-full max-w-sm shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-5 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
              <h3 className="font-black text-gray-800 tracking-tight">ตั้งชื่อเล่น</h3>
              <button onClick={() => setShowNicknameModal(false)} className="p-2 hover:bg-white rounded-full transition shadow-sm">
                <X className="w-5 h-5 text-gray-400" />
              </button>
            </div>
            <div className="p-6 space-y-5">
              <div>
                <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest mb-2 block ml-1">ชื่อเล่นของคุณ</label>
                <input type="text" value={editingMyNickname} onChange={(e) => setEditingMyNickname(e.target.value)}
                  ...
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 text-sm font-bold"
                  style={{ '--tw-ring-color': themeColor } as any} maxLength={30} />
              </div>
              <div>
                <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest mb-2 block ml-1">ชื่อเล่นของคู่สนทนา</label>
                <input type="text" value={editingOtherNickname} onChange={(e) => setEditingOtherNickname(e.target.value)}
                  placeholder={otherUser.display_name}
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 text-sm font-bold"
                  style={{ '--tw-ring-color': themeColor } as any} maxLength={30} />
              </div>
            </div>
            <div className="p-5 bg-gray-50/50 border-t border-gray-100 flex gap-3">
              <button onClick={() => setShowNicknameModal(false)}
                className="flex-1 py-3 bg-white border border-gray-200 hover:bg-gray-100 rounded-2xl text-sm font-bold transition">ยกเลิก</button>
              <button onClick={saveNicknames} disabled={isSavingNickname}
                className="flex-1 py-3 text-white rounded-2xl text-sm font-bold transition disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg"
                style={{ backgroundColor: themeColor }}>
                {isSavingNickname ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />}บันทึก
              </button>
            </div>
          </div>
        </div>
