...
export default function NavLayout({ children }: { children: React.ReactNode }) {
  ...
  const fetchLatestData = async () => {
    try {
      ...
      // ✅ เช็คว่า data มีค่าอยู่จริงก่อนเรียกใช้งาน (ป้องกันหน้าขาว)
    ...
    } catch (err) { console.error("Fetch layout error:", err); }
  };
            ...
            return (
              <Link key={item.label} href={item.href} className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all ${active ? 'bg-frog-500 text-white font-bold' : 'text-gray-500 hover:bg-gray-50'}`}>
                <div className="relative">
                  ...
                  {(item.count ?? 0) > 0 && <span className="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] min-w-[18px] h-[18px] flex items-center justify-center rounded-full border-2 border-white font-black">{item.count! > 99 ? '99+' : item.count}</span>}
                </div>
        ...
        <div className="mt-auto pt-4 border-t border-gray-100">
          {currentUser && (
            <Link href={profileLink} className="flex items-center gap-3 p-2 hover:bg-gray-50 rounded-2xl mb-2">
              <img src={currentUser.profile_img_url || 'https://iili.io/qbtgKBt.png'} className="w-10 h-10 rounded-full object-cover" alt="" />
              <div className="min-w-0 flex-1"><p className="font-bold text-xs truncate text-gray-900">{currentUser.display_name}</p><p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest">My Profile</p></div>
            </Link>
          )}
